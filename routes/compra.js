...
const twilioCl =  require('twilio');

// twilio
const accountSid = config.TWILIO_ACCOUNT_SID;
const authToken = config.TWILIO_AUTH_TOKEN;
const client = twilioCl(accountSid, authToken);

router.post('/agregar', async (req, res) => {
    try {
        const carrito = await compraController.obtenerCompra(req.id);
        const template = carrito.map((item) => `<p>${item.producto.codigo} - ${item.producto.nombre}</p>`).join('');
